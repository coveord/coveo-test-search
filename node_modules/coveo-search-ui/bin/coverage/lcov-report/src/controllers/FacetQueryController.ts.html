...
<html lang="en">
<head>
    <title>Code coverage report for src/controllers/FacetQueryController.ts</title>
    <meta charset="utf-8">
...
<div class="header high">
    <h1>Code coverage report for <span class="entity">src/controllers/FacetQueryController.ts</span></h1>
    <h2>
        Statements: <span class="metric">80.25% <small>(126 / 157)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Branches: <span class="metric">69.32% <small>(61 / 88)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Functions: <span class="metric">70.97% <small>(22 / 31)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Lines: <span class="metric">80.56% <small>(116 / 144)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Ignored: <span class="metric"><span class="ignore-none">none</span></span> &nbsp;&nbsp;&nbsp;&nbsp;
    </h2>
    <div class="path"><a href="../../index.html">All files</a> &#187; <a href="index.html">src/controllers/</a> &#187; FacetQueryController.ts</div>
</div>
<div class="body">
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">/// &lt;reference path='../ui/Facet/Facet.ts' /&gt;
&nbsp;
...
&nbsp;
  ...
  public computeOurFilterExpression(): string {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (selected.length &gt; 0) {
...
&nbsp;
   ...
   */
  public search(params: FacetSearchParameters, <span class="missing-if-branch" title="else path not taken" >E</span>oldLength = params.nbResults): Promise&lt;IIndexFieldValue[]&gt; {
&nbsp;
    return new Promise((resolve, reject) =&gt; {
      let onResult = <span class="fstat-no" title="function not covered" >(fieldValues?: IIndexFieldValue[])</span> =&gt; {
<span class="cstat-no" title="statement not covered" >        let newLength = fieldValues.length;</span>
<span class="cstat-no" title="statement not covered" >        fieldValues = this.checkForFacetSearchValuesToRemove(fieldValues, params.valueToSearch);</span>
<span class="cstat-no" title="statement not covered" >        if (FacetUtils.needAnotherFacetSearch(fieldValues.length, newLength, oldLength, 5)) {</span>
...
<span class="cstat-no" title="statement not covered" >          params.nbResults += 5;</span>
<span class="cstat-no" title="statement not covered" >          return this.search(params, fieldValues.length);</span>
        } else {
<span class="cstat-no" title="statement not covered" >          resolve(fieldValues);</span>
        }
      }
&nbsp;
      this.facet.getEndpoint().search(params.getQuery())
        .then(<span class="fstat-no" title="function not covered" >(queryResults: IQueryResults)</span> =&gt; {
<span class="cstat-no" title="statement not covered" >          if (this.facet.searchInterface.isNewDesign()) {</span>
...
<span class="cstat-no" title="statement not covered" >            let valuesCropped: IGroupByValue[] = [];</span>
<span class="cstat-no" title="statement not covered" >            if (queryResults.groupByResults &amp;&amp; queryResults.groupByResults[0]) {</span>
<span class="cstat-no" title="statement not covered" >              _.each(queryResults.groupByResults[0].values, <span class="fstat-no" title="function not covered" >(v: IGroupByValue)</span> =&gt; {</span>
<span class="cstat-no" title="statement not covered" >                if (v.lookupValue) {</span>
<span class="cstat-no" title="statement not covered" >                  if (!_.contains(params.alwaysExclude, v.lookupValue.toLowerCase())) {</span>
<span class="cstat-no" title="statement not covered" >                    valuesCropped.push(v);</span>
                  }
                } else {
<span class="cstat-no" title="statement not covered" >                  if (!_.contains(params.alwaysExclude, v.value.toLowerCase())) {</span>
<span class="cstat-no" title="statement not covered" >                    valuesCropped.push(v);</span>
                  }
                }
              })
            }
<span class="cstat-no" title="statement not covered" >            onResult(_.first(valuesCropped, params.nbResults));</span>
          } else {
<span class="cstat-no" title="statement not covered" >            resolve(queryResults.groupByResults[0].values);</span>
          }
        })
        .catch(<span class="fstat-no" title="function not covered" >(error: IEndpointError)</span> =&gt; {
...
&nbsp;
  protected createGroupByAllowedValues() {
    ...
    if (this.facet.options.allowedValues != undefined) {
      ...
      let filtered = _.filter(this.getAllowedValuesFromSelected(), <span class="fstat-no" title="function not covered" >(value: string)</span> =&gt; {
<span class="cstat-no" title="statement not covered" >        return !_.contains(toCompare, value.toLowerCase());</span>
      })
...
&nbsp;
  private getAllowedValuesFromSelected() {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.facet.options.useAnd || !this.facet.keepDisplayedValuesNextTime) {
      let selected = this.facet.values.getSelected();
...
<span class="cstat-no" title="statement not covered" >      toMap = this.facet.values.getAll();</span>
    }
...
&nbsp;
    if (this.facet.options.useAnd) {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (Utils.isNonEmptyString(additionalFilter)) {
<span class="cstat-no" title="statement not covered" >        queryOverrideObject = queryBuilder.computeCompleteExpressionParts();</span>
<span class="cstat-no" title="statement not covered" >        if (Utils.isEmptyString(queryOverrideObject.basic)) {</span>
<span class="cstat-no" title="statement not covered" >          queryOverrideObject.basic = '@uri';</span>
        }
      }
    } else {
      if (this.facet.values.hasSelectedOrExcludedValues()) {
        ...
        if (Utils.isNonEmptyString(additionalFilter)) {
          ...
          <span class="missing-if-branch" title="else path not taken" >E</span>if (Utils.isEmptyString(queryOverrideObject.basic)) {
            queryOverrideObject.basic = '@uri';
          }
        }
      }
...
&nbsp;
    if (queryOverrideObject) {
      if (Utils.isNonEmptyString(additionalFilter)) {
        queryOverrideObject.constant = queryOverrideObject.constant ? <span class="branch-0 cbranch-no" title="branch not covered" >queryOverrideObject.constant + ' ' + additionalFilter </span>: additionalFilter;
      }
    }
...
&nbsp;
  protected createBasicGroupByRequest(allowedValues?: string[], <span class="missing-if-branch" title="else path not taken" >E</span>addComputedField: boolean = true): IGroupByRequest {
    let nbOfRequestedValues = this.facet.numberOfValues;
    if (this.facet.options.customSort != null) {
      let usedValues = _.union(_.map(this.facet.values.getSelected(), <span class="fstat-no" title="function not covered" >(facetValue: FacetValue)</span> =&gt; <span class="cstat-no" title="statement not covered" >facetValue.value)</span>, _.map(this.facet.values.getExcluded(), <span class="fstat-no" title="function not covered" >(facetValue: FacetValue)</span> =&gt; <span class="cstat-no" title="statement not covered" >facetValue.value)</span>, this.facet.options.customSort);
...
&nbsp;
<span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >  private checkForFacetSearchValuesToRemove(fieldValues: IIndexFieldValue[], valueToCheckAgainst: string)</span> {</span>
<span class="cstat-no" title="statement not covered" >    let regex = FacetUtils.getRegexToUseForFacetSearch(valueToCheckAgainst, this.facet.options.facetSearchIgnoreAccents);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    return _.filter&lt;IIndexFieldValue&gt;(fieldValues, <span class="fstat-no" title="function not covered" >(fieldValue)</span> =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      let isAllowed =</span>
        _.isEmpty(this.facet.options.allowedValues) ||
        _.contains(this.facet.options.allowedValues, fieldValue.value);
&nbsp;
<span class="cstat-no" title="statement not covered" >      let value = this.facet.getValueCaption(fieldValue);</span>
<span class="cstat-no" title="statement not covered" >      return isAllowed &amp;&amp; regex.test(value);</span>
    })
  }
}
&nbsp;</pre></td></tr>
