...
<html lang="en">
<head>
    <title>Code coverage report for src/ui/Omnibox/Omnibox.ts</title>
    <meta charset="utf-8">
...
<div class="header medium">
    <h1>Code coverage report for <span class="entity">src/ui/Omnibox/Omnibox.ts</span></h1>
    <h2>
        Statements: <span class="metric">75% <small>(246 / 328)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Branches: <span class="metric">60.9% <small>(95 / 156)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Functions: <span class="metric">60.27% <small>(44 / 73)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Lines: <span class="metric">75.42% <small>(227 / 301)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Ignored: <span class="metric"><span class="ignore-none">none</span></span> &nbsp;&nbsp;&nbsp;&nbsp;
    </h2>
    <div class="path"><a href="../../../index.html">All files</a> &#187; <a href="index.html">src/ui/Omnibox/</a> &#187; Omnibox.ts</div>
</div>
<div class="body">
...
707
708
709</td><td class="line-coverage"><span class="cline-any cline-neutral">&nbsp;</span>
...
<span class="cline-any cline-yes">1</span>
<span class="cline-any cline-yes">1</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">///&lt;reference path="FieldAddon.ts" /&gt;
...
&nbsp;
  ...
  constructor(public element: HTMLElement, public options?: IOmniboxOptions, bindings?: IComponentBindings) {
    super(element, Omnibox.ID, bindings)<span class="branch-1 cbranch-no" title="branch not covered" >;</span>
&nbsp;
...
&nbsp;
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (this.options.fieldAlias != null) {
<span class="cstat-no" title="statement not covered" >        this.options.listOfFields = this.options.listOfFields || [];</span>
<span class="cstat-no" title="statement not covered" >        this.options.listOfFields = this.options.listOfFields.concat(_.keys(this.options.fieldAlias));</span>
      }
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.options.grammar != null) {
<span class="cstat-no" title="statement not covered" >      grammar = this.options.grammar(grammar);</span>
    }
&nbsp;
...
&nbsp;
    ...
    this.bind.onRootElement(StandaloneSearchInterfaceEvents.beforeRedirect, <span class="fstat-no" title="function not covered" >() =&gt; <span class="cstat-no" title="statement not covered" ></span>this.handleBeforeRedirect())</span>;
    ...
    if (this.isRevealAutoSuggestion()) {
      this.bind.onRootElement(QueryEvents.duringQuery, <span class="fstat-no" title="function not covered" >(args: IDuringQueryEventArgs)</span> =&gt; <span class="cstat-no" title="statement not covered" >this.handleDuringQuery(args))</span>;
    }
    this.setupMagicBox();
  }
&nbsp;
...
&nbsp;
<span class="fstat-no" title="function not covered" >  public getResult() {</span>
<span class="cstat-no" title="statement not covered" >    return this.magicBox.getResult();</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  public getDisplayedResult() {</span>
<span class="cstat-no" title="statement not covered" >    return this.magicBox.getDisplayedResult();</span>
  }
&nbsp;
  public getCursor() {
...
&nbsp;
  private setupMagicBox() {
    this.magicBox.onmove = <span class="fstat-no" title="function not covered" >() =&gt; {</span>
      // We assume that once the user has moved its selection, it becomes an explicit omnibox analytics event
<span class="cstat-no" title="statement not covered" >      if (this.isRevealAutoSuggestion()) {</span>
<span class="cstat-no" title="statement not covered" >        this.modifyEventTo = this.getOmniboxAnalyticsEventCause();</span>
      }
<span class="cstat-no" title="statement not covered" >      this.movedOnce = true;</span>
    };
&nbsp;
    this.magicBox.onfocus = <span class="fstat-no" title="function not covered" >() =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      if (this.isRevealAutoSuggestion()) {</span>
...
<span class="cstat-no" title="statement not covered" >        this.skipRevealAutoSuggest = true;</span>
      }
    };
&nbsp;
...
&nbsp;
    this.magicBox.onchange = () =&gt; {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (text != undefined &amp;&amp; text != '') {
        if (this.isRevealAutoSuggestion()) {
          <span class="missing-if-branch" title="if path not taken" >I</span>if (this.movedOnce) {
<span class="cstat-no" title="statement not covered" >            this.searchAsYouType(true);</span>
          }
        } else <span class="missing-if-branch" title="else path not taken" >E</span>if (this.options.enableSearchAsYouType) {
...
<span class="cstat-no" title="statement not covered" >      this.magicBox.clearSuggestion();</span>
<span class="cstat-no" title="statement not covered" >      this.updateQueryState();</span>
<span class="cstat-no" title="statement not covered" >      this.triggerNewQuery(false, <span class="fstat-no" title="function not covered" >() =&gt; {</span></span>
<span class="cstat-no" title="statement not covered" >        this.usageAnalytics.logSearchEvent&lt;IAnalyticsNoMeta&gt;(analyticsActionCauseList.searchboxSubmit, {});</span>
      });
<span class="cstat-no" title="statement not covered" >      this.magicBox.blur();</span>
    };
&nbsp;
    this.magicBox.onselect = (suggestion: IOmniboxSuggestion) =&gt; {
      let index = _.indexOf(this.lastSuggestions, suggestion);
      let suggestions = _.compact(_.map(this.lastSuggestions, <span class="fstat-no" title="function not covered" >(suggestion)</span> =&gt; <span class="cstat-no" title="statement not covered" >suggestion.text)</span>);
      this.magicBox.clearSuggestion();
      this.updateQueryState();
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.isRevealAutoSuggestion()) {
<span class="cstat-no" title="statement not covered" >        this.usageAnalytics.cancelAllPendingEvents();</span>
<span class="cstat-no" title="statement not covered" >        this.triggerNewQuery(false, <span class="fstat-no" title="function not covered" >() =&gt; {</span></span>
<span class="cstat-no" title="statement not covered" >          this.usageAnalytics.logSearchEvent&lt;IAnalyticsOmniboxSuggestionMeta&gt;(this.getOmniboxAnalyticsEventCause(), this.buildCustomDataForPartialQueries(index, suggestions));</span>
        });
      } else <span class="missing-if-branch" title="if path not taken" >I</span>if (this.isRevealAutoSuggestion() &amp;&amp; this.movedOnce) {
<span class="cstat-no" title="statement not covered" >        this.handleRevealAutoSuggestionWithKeyboard(index, suggestions);</span>
      } else <span class="missing-if-branch" title="else path not taken" >E</span>if (this.isRevealAutoSuggestion() &amp;&amp; !this.movedOnce) {
...
&nbsp;
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (this.isRevealAutoSuggestion()) {
        this.partialQueries = [];
      }
    };
&nbsp;
    this.magicBox.onblur = <span class="fstat-no" title="function not covered" >() =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      if (this.options.enableSearchAsYouType &amp;&amp; !this.options.inline) {</span>
<span class="cstat-no" title="statement not covered" >        this.setText(this.lastQuery);</span>
      } else {
<span class="cstat-no" title="statement not covered" >        this.updateQueryState();</span>
      }
<span class="cstat-no" title="statement not covered" >      if (this.isRevealAutoSuggestion()) {</span>
<span class="cstat-no" title="statement not covered" >        this.usageAnalytics.sendAllPendingEvents();</span>
      }
    };
&nbsp;
...
&nbsp;
    this.magicBox.ontabpress = <span class="fstat-no" title="function not covered" >() =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.handleTabPress();</span>
    };
&nbsp;
&nbsp;
...
&nbsp;
<span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >  private handleRevealAutoSuggestionWithKeyboard(index: number, suggestions: string[])</span> {</span>
<span class="cstat-no" title="statement not covered" >    if (this.searchAsYouTypeTimeout) {</span>
...
<span class="cstat-no" title="statement not covered" >      this.usageAnalytics.cancelAllPendingEvents();</span>
<span class="cstat-no" title="statement not covered" >      clearTimeout(this.searchAsYouTypeTimeout);</span>
<span class="cstat-no" title="statement not covered" >      this.searchAsYouTypeTimeout = undefined;</span>
<span class="cstat-no" title="statement not covered" >      this.triggerNewQuery(false, <span class="fstat-no" title="function not covered" >() =&gt; {</span></span>
<span class="cstat-no" title="statement not covered" >        this.usageAnalytics.logSearchEvent&lt;IAnalyticsOmniboxSuggestionMeta&gt;(this.getOmniboxAnalyticsEventCause(), this.buildCustomDataForPartialQueries(index, suggestions));</span>
      });
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      this.modifyEventTo = this.getOmniboxAnalyticsEventCause();</span>
<span class="cstat-no" title="statement not covered" >      this.modifyCustomDataOnPending(index, suggestions);</span>
<span class="cstat-no" title="statement not covered" >      this.usageAnalytics.sendAllPendingEvents();</span>
    }
  }
&nbsp;
  private handleRevealAutoSuggestionsWithMouse(index: number, suggestions: string[]) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.searchAsYouTypeTimeout || index != 0) {
...
&nbsp;
<span class="fstat-no" title="function not covered" >  private modifyCustomDataOnPending(index: number, suggestions: string[])</span> {
<span class="cstat-no" title="statement not covered" >    let pendingEvt = this.usageAnalytics.getPendingSearchEvent();</span>
<span class="cstat-no" title="statement not covered" >    if (pendingEvt instanceof PendingSearchAsYouTypeSearchEvent) {</span>
<span class="cstat-no" title="statement not covered" >      let newCustomData = this.buildCustomDataForPartialQueries(index, suggestions);</span>
<span class="cstat-no" title="statement not covered" >      _.each(_.keys(newCustomData), <span class="fstat-no" title="function not covered" >(k: string)</span> =&gt; {</span>
        (&lt;PendingSearchAsYouTypeSearchEvent&gt;<span class="cstat-no" title="statement not covered" >pendingEvt).modifyCustomData(k, newCustomData[k]);</span>
      });
    }
  }
&nbsp;
...
&nbsp;
  private cleanCustomData(toClean: string[], <span class="missing-if-branch" title="else path not taken" >E</span>rejectLength = 256) {
    ...
    toClean = _.compact(_.filter(toClean, (partial: string, pos?: number, array?: string[]) =&gt; {
      return pos === 0 || <span class="branch-1 cbranch-no" title="branch not covered" >partial !== array[pos - 1];</span>
    }));
&nbsp;
...
&nbsp;
    ...
    _.reduceRight(toClean, (memo: number, partial: string) =&gt; {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (totalSoFar &lt;= rejectLength) {
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (ret.length &gt;= 256) {
<span class="cstat-no" title="statement not covered" >      return this.cleanCustomData(toClean, rejectLength - 10);</span>
    }
&nbsp;
    return toClean.join(';');
  }
...
&nbsp;
<span class="fstat-no" title="function not covered" >  private handleBeforeRedirect() {</span>
<span class="cstat-no" title="statement not covered" >    this.updateQueryState();</span>
  }
&nbsp;
...
&nbsp;
    let result: Coveo.MagicBox.Result = this.lastQuery == this.magicBox.getDisplayedResult().input ? this.magicBox.getDisplayedResult().clone() : <span class="branch-1 cbranch-no" title="branch not covered" >this.magicBox.grammar.parse(this.lastQuery).clean();</span>
...
&nbsp;
    if (this.options.enableQuerySyntax) {
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (this.options.fieldAlias) {
<span class="cstat-no" title="statement not covered" >        let fieldNames = preprocessResultForQueryArgs.result.findAll(<span class="fstat-no" title="function not covered" >(result: Coveo.MagicBox.Result)</span> =&gt; <span class="cstat-no" title="statement not covered" >result.expression.id == 'FieldName' &amp;&amp; result.isSuccess())</span>;</span>
<span class="cstat-no" title="statement not covered" >        _.each(fieldNames, <span class="fstat-no" title="function not covered" >(result: Coveo.MagicBox.Result)</span> =&gt; {</span>
<span class="cstat-no" title="statement not covered" >          let alias = _.find(_.keys(this.options.fieldAlias), <span class="fstat-no" title="function not covered" >(alias: string)</span> =&gt; <span class="cstat-no" title="statement not covered" >alias.toLowerCase() == result.value.toLowerCase())</span>;</span>
<span class="cstat-no" title="statement not covered" >          if (alias != null) {</span>
<span class="cstat-no" title="statement not covered" >            result.value = &lt;string&gt;this.options.fieldAlias[alias];</span>
          }
        });
      }
    }
...
&nbsp;
<span class="fstat-no" title="function not covered" >  private handleTabPress() {</span>
<span class="cstat-no" title="statement not covered" >    if (this.options.enableRevealQuerySuggestAddon) {</span>
<span class="cstat-no" title="statement not covered" >      this.handleTabPressForRevealSuggestions();</span>
    } else {
<span class="cstat-no" title="statement not covered" >      this.handleTabPressForOldOmniboxAddon();</span>
    }
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  private handleTabPressForRevealSuggestions() {</span>
<span class="cstat-no" title="statement not covered" >    if (!this.options.enableSearchAsYouType) {</span>
<span class="cstat-no" title="statement not covered" >      let suggestions = _.compact(_.map(this.lastSuggestions, <span class="fstat-no" title="function not covered" >(suggestion)</span> =&gt; <span class="cstat-no" title="statement not covered" >suggestion.text)</span>);</span>
<span class="cstat-no" title="statement not covered" >      this.usageAnalytics.logCustomEvent(this.getOmniboxAnalyticsEventCause(), this.buildCustomDataForPartialQueries(0, suggestions), this.element);</span>
    }
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  private handleTabPressForOldOmniboxAddon() {</span>
<span class="cstat-no" title="statement not covered" >    if (this.lastSuggestions &amp;&amp; this.lastSuggestions[0] &amp;&amp; this.lastSuggestions[0].dom) {</span>
<span class="cstat-no" title="statement not covered" >      let firstSelected = $$(this.lastSuggestions[0].dom).find('.coveo-omnibox-selected');</span>
<span class="cstat-no" title="statement not covered" >      let firstSelectable = $$(this.lastSuggestions[0].dom).find('.coveo-omnibox-selectable');</span>
<span class="cstat-no" title="statement not covered" >      if (firstSelected) {</span>
<span class="cstat-no" title="statement not covered" >        $$(firstSelected).trigger('tabSelect');</span>
      } else <span class="cstat-no" title="statement not covered" >if (firstSelectable) {</span>
<span class="cstat-no" title="statement not covered" >        $$(firstSelectable).trigger('tabSelect');</span>
      }
    }
  }
&nbsp;
  private triggerNewQuery(searchAsYouType: boolean, analyticsEvent: () =&gt; void) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.shouldExecuteQuery(searchAsYouType)) {
      this.lastQuery = text;
      analyticsEvent();
  ...
  private getQuery(searchAsYouType: boolean) {
    ...
    if (searchAsYouType) {
      ...
      if (query == null &amp;&amp; this.lastSuggestions != null &amp;&amp; this.lastSuggestions.length &gt; 0) {
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if (textSuggestion != null) {
...
&nbsp;
<span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >  private handleDuringQuery(args: IDuringQueryEventArgs)</span> {</span>
    // When the query results returns ... (args.promise)
<span class="cstat-no" title="statement not covered" >    args.promise.then(<span class="fstat-no" title="function not covered" >() =&gt; {</span></span>
      // Get a handle on a pending search as you type (those events are delayed, not sent instantly)
<span class="cstat-no" title="statement not covered" >      let pendingEvent = this.usageAnalytics.getPendingSearchEvent();</span>
<span class="cstat-no" title="statement not covered" >      if (pendingEvent instanceof PendingSearchAsYouTypeSearchEvent) {</span>
        (&lt;PendingSearchAsYouTypeSearchEvent&gt;<span class="cstat-no" title="statement not covered" >pendingEvent).beforeResolve.then(<span class="fstat-no" title="function not covered" >(evt)</span> =&gt; {</span>
          // Check if we need to modify the event type beforeResolving it
<span class="cstat-no" title="statement not covered" >          args.promise.then(<span class="fstat-no" title="function not covered" >() =&gt; {</span></span>
<span class="cstat-no" title="statement not covered" >            if (this.modifyEventTo) {</span>
<span class="cstat-no" title="statement not covered" >              evt.modifyEventCause(this.modifyEventTo);</span>
<span class="cstat-no" title="statement not covered" >              this.modifyEventTo = null;</span>
            }
          });
        });
      }
...
&nbsp;
  private suggestionShouldTriggerQuery(<span class="missing-if-branch" title="else path not taken" >E</span>suggestions = this.lastSuggestions) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.shouldExecuteQuery(true)) {
...
&nbsp;
  private getOmniboxAnalyticsEventCause() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.searchInterface instanceof StandaloneSearchInterface) {
<span class="cstat-no" title="statement not covered" >      return analyticsActionCauseList.omniboxFromLink;</span>
    }
