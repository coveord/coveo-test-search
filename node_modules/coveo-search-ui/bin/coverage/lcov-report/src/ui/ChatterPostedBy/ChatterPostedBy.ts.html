...
<html lang="en">
<head>
    <title>Code coverage report for src/ui/ChatterPostedBy/ChatterPostedBy.ts</title>
    <meta charset="utf-8">
...
<div class="header high">
    <h1>Code coverage report for <span class="entity">src/ui/ChatterPostedBy/ChatterPostedBy.ts</span></h1>
    <h2>
        ...
        Lines: <span class="metric">88.57% <small>(31 / 35)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Ignored: <span class="metric"><span class="ignore-none">none</span></span> &nbsp;&nbsp;&nbsp;&nbsp;
    </h2>
    <div class="path"><a href="../../../index.html">All files</a> &#187; <a href="index.html">src/ui/ChatterPostedBy/</a> &#187; ChatterPostedBy.ts</div>
</div>
<div class="body">
...
&nbsp;
  constructor(public element: HTMLElement, public options?: IChatterPostedByOption, public bindings?: IResultsComponentBindings, public result?: IQueryResult) {
    super(element, ChatterPostedBy.ID, bindings)<span class="branch-1 cbranch-no" title="branch not covered" >;</span>
    this.options = ComponentOptions.initComponentOptions(element, ChatterPostedBy, options);
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (Utils.getFieldValue(this.result, 'sfcreatedby') != null) {
      let from = $$('span');
      from.text(`${this.options.useFromInstead ? <span class="branch-0 cbranch-no" title="branch not covered" >l('From') </span>: l('PostedBy')} `);
...
&nbsp;
      if (this.options.enablePostedOn &amp;&amp; !Utils.isNullOrUndefined(Utils.getFieldValue(this.result, 'sfparentname')) &amp;&amp; !Utils.isNullOrUndefined(Utils.getFieldValue(this.result, 'sfparentid'))) {
        // Post on user's wall
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!Utils.isNullOrUndefined(Utils.getFieldValue(this.result, 'sfuserid')) &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >Utils.getFieldValue(this.result, 'sfuserid') != Utils.getFieldValue(this.result, 'sfinsertedbyid'))</span> {
<span class="cstat-no" title="statement not covered" >          let onFeed = $$('span');</span>
<span class="cstat-no" title="statement not covered" >          let content = ` ${l('OnFeed', this.renderLink(Utils.getFieldValue(this.result, 'sfparentname'), Utils.getFieldValue(this.result, 'sfparentid')).outerHTML)}`;</span>
<span class="cstat-no" title="statement not covered" >          onFeed.el.innerHTML = content;</span>
<span class="cstat-no" title="statement not covered" >          $$(element).append(onFeed.el);</span>
&nbsp;
        } else <span class="missing-if-branch" title="else path not taken" >E</span>if (Utils.isNullOrUndefined(Utils.getFieldValue(this.result, 'sfuserid'))) {
          let onUser = $$('span');
