...
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (result.raw.sfcreatedby != null) {
      let from = $$('span');
...
&nbsp;
      if (this.options.enablePostedOn &amp;&amp; !Utils.isNullOrUndefined(result.raw.sfparentname) &amp;&amp; !Utils.isNullOrUndefined(result.raw.sfparentid)) {
        // Post on user's wall
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!Utils.isNullOrUndefined(result.raw.sfuserid) &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >result.raw.sfuserid != result.raw.sfinsertedbyid)</span> {
<span class="cstat-no" title="statement not covered" >          let onFeed = $$('span');</span>
<span class="cstat-no" title="statement not covered" >          let content = ` ${l('OnFeed', this.renderLink(result.raw.sfparentname, result.raw.sfparentid).outerHTML)}`;</span>
...
&nbsp;
        } else <span class="missing-if-branch" title="else path not taken" >E</span>if (Utils.isNullOrUndefined(result.raw.sfuserid)) {
          let onUser = $$('span');
