...
<html lang="en">
<head>
    <title>Code coverage report for src/ui/ImageResultList/ImageResultList.ts</title>
    <meta charset="utf-8">
...
<div class="header high">
    <h1>Code coverage report for <span class="entity">src/ui/ImageResultList/ImageResultList.ts</span></h1>
    <h2>
        Statements: <span class="metric">87.57% <small>(155 / 177)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Branches: <span class="metric">52.17% <small>(24 / 46)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Functions: <span class="metric">85.71% <small>(30 / 35)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Lines: <span class="metric">87.35% <small>(145 / 166)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Ignored: <span class="metric"><span class="ignore-none">none</span></span> &nbsp;&nbsp;&nbsp;&nbsp;
    </h2>
    <div class="path"><a href="../../../index.html">All files</a> &#187; <a href="index.html">src/ui/ImageResultList/</a> &#187; ImageResultList.ts</div>
</div>
<div class="body">
...
318
319
320</td><td class="line-coverage"><span class="cline-any cline-yes">1</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { ResultList, IResultListOptions } from '../ResultList/ResultList';
...
&nbsp;
   ...
   */
  constructor(public element: HTMLElement, public options?: IImageResultListOptions, bindings?: IComponentBindings, <span class="missing-if-branch" title="else path not taken" >E</span>elementClassId: string = ResultList.ID) {
&nbsp;
    super(element, options, bindings, ImageResultList.ID)<span class="branch-1 cbranch-no" title="branch not covered" >;</span>
&nbsp;
...
&nbsp;
<span class="fstat-no" title="function not covered" >  private resize() {</span>
...
<span class="cstat-no" title="statement not covered" >    timeout = setTimeout(this.retrieveLayoutMethod(), 250);</span>
  }
&nbsp;
  private handleProcessNewResultsDisplayed() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.disabled) {
<span class="cstat-no" title="statement not covered" >      $$(this.element).hide();</span>
    } else {
      $$(this.element).show();
    }
...
&nbsp;
  private retrieveLayoutMethod(): () =&gt; void {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.disabled) {
      ...
      } else <span class="missing-if-branch" title="else path not taken" >E</span>if (this.options.layoutType.toLowerCase() == ImageResultList.rowLayoutTypeStr) {
...
&nbsp;
  private onImageProxy(callback: () =&gt; void) {
    _.each(this.getResultsElement(), (element: HTMLElement) =&gt; {
      <span class="missing-if-branch" title="if path not taken" >I</span>if ($$(element).findAll('img').length == 0) {
...
&nbsp;
    ...
    let onImageLoad = (image: HTMLImageElement) =&gt; {
      <span class="missing-if-branch" title="else path not taken" >E</span>if ($$(image).height() &gt; 0) {
        <span class="missing-if-branch" title="if path not taken" >I</span>if ($$(image).getAttribute('width') == null &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >$$(image).getAttribute('height') == null)</span> {
<span class="cstat-no" title="statement not covered" >          $$(image).setAttribute('height', $$(image).height().toString());</span>
<span class="cstat-no" title="statement not covered" >          $$(image).setAttribute('width', $$(image).width().toString());</span>
        }
        loaded++;
...
<span class="cstat-no" title="statement not covered" >        $$(image.parentElement).detach();</span>
<span class="cstat-no" title="statement not covered" >        images = _.filter(images, <span class="fstat-no" title="function not covered" >(img)</span> =&gt; {</span>
<span class="cstat-no" title="statement not covered" >          return !img.isEqualNode(image);</span>
        });
<span class="cstat-no" title="statement not covered" >        if (loaded == images.length) {</span>
<span class="cstat-no" title="statement not covered" >          callback();</span>
        }
      }
    };
    _.each(images, (e: HTMLImageElement) =&gt; {
      <span class="missing-if-branch" title="else path not taken" >E</span>if ((e.src &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >e.complete)</span> || /*for IE 10-*/ $$(e).height() &gt; 0) {
        onImageLoad(e);
      } else {
<span class="cstat-no" title="statement not covered" >        $$(e).one('load', <span class="fstat-no" title="function not covered" >() =&gt; <span class="cstat-no" title="statement not covered" ></span>onImageLoad(e))</span>;</span>
      }
    });
  }
&nbsp;
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.options.enableInfiniteScroll) {
<span class="cstat-no" title="statement not covered" >      this.adjustNumberOfResults();</span>
    }
  }
&nbsp;
...
&nbsp;
  private getCurrentRowHeight(results: HTMLElement[], containerWidth: number): number {
    ...
    while ((this.resultIndex &lt; results.length) &amp;&amp; (height == null || height &gt;= this.options.heightThreshold)) {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (imageDimensions != null) {
...
&nbsp;
<span class="fstat-no" title="function not covered" >  private adjustNumberOfResults() {</span>
<span class="cstat-no" title="statement not covered" >    let elementHeight = Number($$(this.element).getAttribute('data-height'));</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (_.isNumber(elementHeight) == false) {</span>
<span class="cstat-no" title="statement not covered" >      elementHeight = $$(this.element).height();</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if ((elementHeight &lt; $$(&lt;HTMLElement&gt;this.options.infiniteScrollContainer).height()) &amp;&amp; super.hasPotentiallyMoreResultsToDisplay()) {</span>
<span class="cstat-no" title="statement not covered" >      super.displayMoreResults(this.options.infiniteScrollPageSize);</span>
    }
  }
}
&nbsp;
