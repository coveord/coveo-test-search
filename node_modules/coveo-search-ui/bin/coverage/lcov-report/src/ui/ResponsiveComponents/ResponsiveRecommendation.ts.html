...
<html lang="en">
<head>
    <title>Code coverage report for src/ui/ResponsiveComponents/ResponsiveRecommendation.ts</title>
    <meta charset="utf-8">
...
<div class="header high">
    <h1>Code coverage report for <span class="entity">src/ui/ResponsiveComponents/ResponsiveRecommendation.ts</span></h1>
    <h2>
        Statements: <span class="metric">87.1% <small>(135 / 155)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Branches: <span class="metric">52.78% <small>(19 / 36)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Functions: <span class="metric">77.14% <small>(27 / 35)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Lines: <span class="metric">88.19% <small>(127 / 144)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Ignored: <span class="metric"><span class="ignore-none">none</span></span> &nbsp;&nbsp;&nbsp;&nbsp;
    </h2>
    <div class="path"><a href="../../../index.html">All files</a> &#187; <a href="index.html">src/ui/ResponsiveComponents/</a> &#187; ResponsiveRecommendation.ts</div>
</div>
<div class="body">
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { ResponsiveComponentsManager, IResponsiveComponent, IResponsiveComponentOptions } from './ResponsiveComponentsManager';
...
&nbsp;
  public static init(root: HTMLElement, component, options: IResponsiveComponentOptions) {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!coveoRoot) {
      logger.info('Recommendation component has no parent interface. Disabling responsive mode for this component.');
      return;
    }
<span class="cstat-no" title="statement not covered" >    if (!$$(coveoRoot).find('.coveo-results-column')) {</span>
<span class="cstat-no" title="statement not covered" >      logger.info('Cannot find element with class coveo-results-column. Disabling responsive mode for this component.');</span>
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    ResponsiveComponentsManager.register(ResponsiveRecommendation, $$(coveoRoot), Recommendation.ID, component, options);</span>
  }
&nbsp;
  private static findParentRootOfRecommendationComponent(root: HTMLElement): Dom {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (coveoRoot[0]) {
<span class="cstat-no" title="statement not covered" >      return $$(coveoRoot[0]);</span>
    }
    return null;
  }
&nbsp;
...
&nbsp;
  public handleResizeEvent(): void {
    ...
    } else <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.needSmallMode() &amp;&amp; ResponsiveComponentsUtils.isSmallRecommendationActivated(this.coveoRoot)) {
...
&nbsp;
  private buildDropdown(responsiveDropdown?: ResponsiveDropdown): ResponsiveDropdown {
    ...
    let dropdown = responsiveDropdown ? responsiveDropdown : <span class="branch-1 cbranch-no" title="branch not covered" >new ResponsiveDropdown(dropdownContent, dropdownHeader, this.coveoRoot);</span>
...
&nbsp;
  private buildDropdownContent(): RecommendationDropdownContent {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (recommendationColumn) {
<span class="cstat-no" title="statement not covered" >      dropdownContentElement = $$(recommendationColumn);</span>
    } else {
...
&nbsp;
  private defineResponsiveBreakpoint(options: IResponsiveComponentOptions): number {
    let breakpoint;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (Utils.isNullOrUndefined(options.responsiveBreakpoint)) {
      breakpoint = ResponsiveRecommendation.RESPONSIVE_BREAKPOINT;
    } else {
<span class="cstat-no" title="statement not covered" >      breakpoint = options.responsiveBreakpoint;</span>
    }
    return breakpoint;
  }
&nbsp;
  private getFacetSliders(): FacetSlider[] {
    let facetSliders = [];
    _.each(this.coveoRoot.findAll('.' + Component.computeCssClassName(FacetSlider)), <span class="fstat-no" title="function not covered" >facetSliderElement </span>=&gt; {
<span class="cstat-no" title="statement not covered" >      let facetSlider = Component.get(facetSliderElement, FacetSlider);</span>
<span class="cstat-no" title="statement not covered" >      if (facetSlider instanceof FacetSlider) {</span>
<span class="cstat-no" title="statement not covered" >        facetSliders.push(facetSlider);</span>
      }
    });
    return facetSliders;
  }
&nbsp;
  private getFacets(): Facet[] {
    let facets = [];
    _.each(this.coveoRoot.findAll('.' + Component.computeCssClassName(Facet)), <span class="fstat-no" title="function not covered" >facetElement </span>=&gt; {
<span class="cstat-no" title="statement not covered" >      let facet = Component.get(facetElement, Facet);</span>
<span class="cstat-no" title="statement not covered" >      if (facet instanceof Facet) {</span>
<span class="cstat-no" title="statement not covered" >        facets.push(facet);</span>
      }
    });
    return facets;
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  private dismissFacetSearches(): void {</span>
...
&nbsp;
  private enableFacetPreservePosition(): void {
    _.each(this.facets, <span class="fstat-no" title="function not covered" >facet </span>=&gt; <span class="cstat-no" title="statement not covered" >facet.options.preservePosition = true)</span>;
  }
&nbsp;
  private disableFacetPreservePosition(): void {
    _.each(this.facets, <span class="fstat-no" title="function not covered" >facet </span>=&gt; <span class="cstat-no" title="statement not covered" >facet.options.preservePosition = false)</span>;
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  private drawFacetSliderGraphs(): void {</span>
...
&nbsp;
  private registerQueryEvents() {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (recommendationInstance &amp;&amp; recommendationInstance.options.hideIfNoResults) {
