...
<div class="header high">
    ...
    <h2>
        Statements: <span class="metric">94.97% <small>(151 / 159)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Branches: <span class="metric">69.39% <small>(34 / 49)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Functions: <span class="metric">97.14% <small>(34 / 35)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Lines: <span class="metric">94.56% <small>(139 / 147)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
...
&nbsp;
  constructor(public coveoRoot: Dom, public ID: string, options: IResponsiveComponentOptions, responsiveDropdown?: ResponsiveDropdown) {
    ...
    if (Utils.isNullOrUndefined(options.responsiveBreakpoint)) {
      this.breakpoint = this.searchInterface ? <span class="branch-0 cbranch-no" title="branch not covered" >this.searchInterface.responsiveComponents.getMediumScreenWidth() </span>: new ResponsiveComponents().getMediumScreenWidth();
    } else {
...
&nbsp;
  public registerComponent(accept: Component) {
    if (accept instanceof Facet) {
      ...
      return true;
    } else <span class="missing-if-branch" title="else path not taken" >E</span>if (accept instanceof FacetSlider) {
...
&nbsp;
  public handleResizeEvent() {
    ...
    } else <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.needSmallMode() &amp;&amp; ResponsiveComponentsUtils.isSmallFacetActivated(this.coveoRoot)) {
...
&nbsp;
  public dismissFacetSearches() {
    _.each(this.facets, facet =&gt; {
      <span class="missing-if-branch" title="else path not taken" >E</span>if (facet.facetSearch &amp;&amp; facet.facetSearch.currentlyDisplayedResults) {
  ...
  private bindDropdownContentEvents() {
    this.dropdown.dropdownContent.element.on('scroll', _.debounce(() =&gt; {
      _.each(this.facets, facet =&gt; {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (facetSearch &amp;&amp; facetSearch.currentlyDisplayedResults &amp;&amp; !this.isFacetSearchScrolledIntoView(facetSearch.search)) {
<span class="cstat-no" title="statement not covered" >          facet.facetSearch.positionSearchResults(this.dropdown.dropdownContent.element.el);</span>
        } else <span class="missing-if-branch" title="else path not taken" >E</span>if (facetSearch &amp;&amp; facet.facetSearch.currentlyDisplayedResults) {
...
&nbsp;
    dropdownTop = dropdownTop &gt;= 0 ? dropdownTop : <span class="branch-1 cbranch-no" title="branch not covered" >0;</span>
&nbsp;
...
&nbsp;
  private getDropdownHeaderLabel() {
    ...
    _.each($$(this.coveoRoot.find('.coveo-facet-column')).findAll(selector), <span class="fstat-no" title="function not covered" >facetElement </span>=&gt; {
<span class="cstat-no" title="statement not covered" >      let facet: Facet | FacetSlider;</span>
<span class="cstat-no" title="statement not covered" >      if ($$(facetElement).hasClass(Component.computeCssClassName(Facet))) {</span>
<span class="cstat-no" title="statement not covered" >        facet = &lt;Facet&gt;Component.get(facetElement, Facet);</span>
      } else {
<span class="cstat-no" title="statement not covered" >        facet = &lt;FacetSlider&gt;Component.get(facetElement, FacetSlider);</span>
      }
<span class="cstat-no" title="statement not covered" >      if (!dropdownHeaderLabel &amp;&amp; facet.options.dropdownHeaderLabel) {</span>
<span class="cstat-no" title="statement not covered" >        dropdownHeaderLabel = facet.options.dropdownHeaderLabel;</span>
      }
    });
&nbsp;
