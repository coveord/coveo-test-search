...
<html lang="en">
<head>
    <title>Code coverage report for src/ui/ResultList/ResultList.ts</title>
    <meta charset="utf-8">
...
<div class="header high">
    <h1>Code coverage report for <span class="entity">src/ui/ResultList/ResultList.ts</span></h1>
    <h2>
        Statements: <span class="metric">80.08% <small>(201 / 251)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Branches: <span class="metric">58.82% <small>(40 / 68)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Functions: <span class="metric">70.91% <small>(39 / 55)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Lines: <span class="metric">80.09% <small>(185 / 231)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        Ignored: <span class="metric"><span class="ignore-none">none</span></span> &nbsp;&nbsp;&nbsp;&nbsp;
    </h2>
    <div class="path"><a href="../../../index.html">All files</a> &#187; <a href="index.html">src/ui/ResultList/</a> &#187; ResultList.ts</div>
</div>
<div class="body">
...
&nbsp;
    if (this.options.enableInfiniteScroll) {
      this.bind.on(&lt;HTMLElement&gt;this.options.infiniteScrollContainer, 'scroll', <span class="fstat-no" title="function not covered" >(e: Event)</span> =&gt; <span class="cstat-no" title="statement not covered" >this.handleScrollOfResultList())</span>;
    }
    this.bind.onQueryState(MODEL_EVENTS.CHANGE_ONE, QUERY_STATE_ATTRIBUTES.FIRST, <span class="fstat-no" title="function not covered" >() =&gt; <span class="cstat-no" title="statement not covered" ></span>this.handlePageChanged())</span>;
  }
&nbsp;
  /**
   ...
   * @param append
   */
  public renderResults(resultsElement: HTMLElement[], <span class="missing-if-branch" title="else path not taken" >E</span>append = false) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!append) {
...
&nbsp;
  ...
  public buildResults(results: IQueryResults): HTMLElement[] {
    ...
    _.each(results.results, (result: IQueryResult) =&gt; {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (resultElement != null) {
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.isCurrentlyFetchingMoreResults()) {
<span class="cstat-no" title="statement not covered" >      this.logger.warn('Ignoring request to display more results since we\'re already doing so');</span>
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.hasPotentiallyMoreResultsToDisplay()) {
<span class="cstat-no" title="statement not covered" >      this.logger.warn('Ignoring request to display more results since we know there aren\'t more to display');</span>
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.options.enableInfiniteScrollWaitingAnimation) {
      this.showWaitingAnimationForInfiniteScrolling();
    }
&nbsp;
    this.fetchingMoreResults = this.queryController.fetchMore(count);
    this.fetchingMoreResults.then(<span class="fstat-no" title="function not covered" >(data: IQueryResults)</span> =&gt; {
<span class="cstat-no" title="statement not covered" >      Assert.exists(data);</span>
<span class="cstat-no" title="statement not covered" >      this.usageAnalytics.logCustomEvent&lt;IAnalyticsNoMeta&gt;(analyticsActionCauseList.pagerScrolling, {}, this.element);</span>
<span class="cstat-no" title="statement not covered" >      var results = data.results;</span>
<span class="cstat-no" title="statement not covered" >      this.reachedTheEndOfResults = count &gt; data.results.length;</span>
<span class="cstat-no" title="statement not covered" >      this.renderResults(this.buildResults(data), true);</span>
<span class="cstat-no" title="statement not covered" >      _.each(results, <span class="fstat-no" title="function not covered" >(result)</span> =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        this.currentlyDisplayedResults.push(result);</span>
      });
<span class="cstat-no" title="statement not covered" >      this.triggerNewResultsDisplayed();</span>
    });
&nbsp;
    this.fetchingMoreResults.then(<span class="fstat-no" title="function not covered" >() =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.hideWaitingAnimationForInfiniteScrolling();</span>
<span class="cstat-no" title="statement not covered" >      this.fetchingMoreResults = undefined;</span>
<span class="cstat-no" title="statement not covered" >      Defer.defer(<span class="fstat-no" title="function not covered" >() =&gt; <span class="cstat-no" title="statement not covered" ></span>this.handleScrollOfResultList())</span>;</span>
    });
  }
&nbsp;
  /**
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (DeviceUtils.isMobileDevice() &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >this.options.mobileScrollContainer != undefined)</span> {
<span class="cstat-no" title="statement not covered" >        this.options.mobileScrollContainer.scrollTop = 0;</span>
      }
    }
  }
&nbsp;
  private handleScrollOfResultList() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.isCurrentlyFetchingMoreResults() || !this.options.enableInfiniteScroll) {
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.isScrollingOfResultListAlmostAtTheBottom() &amp;&amp; this.hasPotentiallyMoreResultsToDisplay()) {
      this.displayMoreResults(this.options.infiniteScrollPageSize);
    }
  }
&nbsp;
<span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >  private handlePageChanged() {</span></span>
<span class="cstat-no" title="statement not covered" >    this.bind.oneRootElement(QueryEvents.querySuccess, <span class="fstat-no" title="function not covered" >() =&gt; {</span></span>
<span class="cstat-no" title="statement not covered" >      if (this.options.infiniteScrollContainer instanceof Window) {</span>
<span class="cstat-no" title="statement not covered" >        var win = &lt;Window&gt;this.options.infiniteScrollContainer;</span>
<span class="cstat-no" title="statement not covered" >        win.scrollTo(0, 0);</span>
      } else {
<span class="cstat-no" title="statement not covered" >        var el = &lt;HTMLElement&gt;this.options.infiniteScrollContainer;</span>
<span class="cstat-no" title="statement not covered" >        if (el.scrollIntoView) {</span>
<span class="cstat-no" title="statement not covered" >          el.scrollIntoView();</span>
        }
      }
    })
  }
...
&nbsp;
  private handleBuildingQuery(args: IBuildingQueryEventArgs) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.options.autoSelectFieldsToInclude) {
<span class="cstat-no" title="statement not covered" >      args.queryBuilder.addRequiredFields(this.getAutoSelectedFieldsToInclude());</span>
<span class="cstat-no" title="statement not covered" >      args.queryBuilder.includeRequiredFields = true;</span>
    }
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  private getAutoSelectedFieldsToInclude() {</span>
<span class="cstat-no" title="statement not covered" >    return _.chain(this.options.resultTemplate.getFields())</span>
      .compact()
      .unique()
      .value()
  }
...
<span class="cstat-no" title="statement not covered" >      isWindow = false;</span>
    }
    return isWindow ? this.isScrollAtBottomForWindowElement() : <span class="branch-1 cbranch-no" title="branch not covered" >this.isScrollAtBottomForHtmlElement();</span>
  }
&nbsp;
...
&nbsp;
<span class="fstat-no" title="function not covered" >  private isScrollAtBottomForHtmlElement() {</span>
<span class="cstat-no" title="statement not covered" >    var el = &lt;HTMLElement&gt;this.options.infiniteScrollContainer;</span>
<span class="cstat-no" title="statement not covered" >    var elementHeight = el.clientHeight;</span>
<span class="cstat-no" title="statement not covered" >    var scrollHeight = el.scrollHeight;</span>
<span class="cstat-no" title="statement not covered" >    var bottomPosition = el.scrollTop + elementHeight;</span>
<span class="cstat-no" title="statement not covered" >    return (scrollHeight - bottomPosition) &lt; elementHeight / 2;</span>
  }
&nbsp;
...
&nbsp;
  private cancelFetchingMoreResultsIfNeeded() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.isCurrentlyFetchingMoreResults()) {
<span class="cstat-no" title="statement not covered" >      this.logger.trace('Cancelling fetching more results');</span>
<span class="cstat-no" title="statement not covered" >      Promise.reject(this.fetchingMoreResults);</span>
<span class="cstat-no" title="statement not covered" >      this.fetchingMoreResults = undefined;</span>
    }
  }
&nbsp;
...
&nbsp;
    _.each(showIfQuery, <span class="fstat-no" title="function not covered" >(s: HTMLElement)</span> =&gt; {
<span class="cstat-no" title="statement not covered" >      $$(s).toggle(hasQuery);</span>
    })
    _.each(showIfNoQuery, <span class="fstat-no" title="function not covered" >(s: HTMLElement)</span> =&gt; {
<span class="cstat-no" title="statement not covered" >      $$(s).toggle(!hasQuery);</span>
    })
    _.each(showIfResults, <span class="fstat-no" title="function not covered" >(s: HTMLElement)</span> =&gt; {
<span class="cstat-no" title="statement not covered" >      $$(s).toggle(hasQuery &amp;&amp; hasResults);</span>
    })
    _.each(showIfNoResults, <span class="fstat-no" title="function not covered" >(s: HTMLElement)</span> =&gt; {
<span class="cstat-no" title="statement not covered" >      $$(s).toggle(hasQuery &amp;&amp; !hasResults);</span>
    })
  }
&nbsp;
  private showWaitingAnimation() {
    switch (this.options.waitAnimation.toLowerCase()) {
      case 'fade':
        ...
        break;
      case 'spinner':
        _.each(this.options.resultContainer.children, <span class="fstat-no" title="function not covered" >(child: HTMLElement)</span> =&gt; {
<span class="cstat-no" title="statement not covered" >          $$(child).hide();</span>
        })
        <span class="missing-if-branch" title="else path not taken" >E</span>if ($$(this.options.waitAnimationContainer).find('.coveo-wait-animation') == undefined) {
  ...
  private hideWaitingAnimation() {
    switch (this.options.waitAnimation.toLowerCase()) {
      ...
      case 'spinner':
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (spinner) {
<span class="cstat-no" title="statement not covered" >          $$(spinner).detach();</span>
        }
        break;
    }
  }
...
&nbsp;
<span class="fstat-no" title="function not covered" >  private hideWaitingAnimationForInfiniteScrolling() {</span>
<span class="cstat-no" title="statement not covered" >    var spinner = $$(this.options.waitAnimationContainer).find('.coveo-loading-spinner');</span>
...
&nbsp;
  private static getDefaultTemplate(e: HTMLElement): Template {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (component.searchInterface instanceof Recommendation) {
<span class="cstat-no" title="statement not covered" >      return new DefaultRecommendationTemplate();</span>
    }
